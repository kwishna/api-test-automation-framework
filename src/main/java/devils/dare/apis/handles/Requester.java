...
public class Requester {
    ...
    private Response makeGetRequest(String path) {
        ...
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).get(path).thenReturn();
    }

    private Response makePostRequest(String path) {
        LOGGER.debug("Making 'POST' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).post(path).thenReturn();
    }

    private Response makePutRequest(String path) {
        LOGGER.debug("Making 'PUT' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).put(path).thenReturn();
    }

    private Response makePatchRequest(String path) {
        LOGGER.debug("Making 'PATCH' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).patch(path).thenReturn();
    }

    private Response makeHeadRequest(String path) {
        LOGGER.debug("Making 'HEAD' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).head(path).thenReturn();
    }

    private Response makeOptionsRequest(String path) {
        LOGGER.debug("Making 'OPTIONS' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).options(path).thenReturn();
    }

    private Response makeDeleteRequest(String path) {
        LOGGER.debug("Making 'DELETE' request");
        return SerenityRest.given().spec(BaseEndpoint.getRequestSpecMngr().getSpec()).delete(path).thenReturn();
    }
